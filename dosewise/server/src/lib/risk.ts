...
const SEDATING_CATEGORIES = [
  "antihistamine", "benzodiazepine", "opioid", "sleep aid", "muscle relaxant", "antidepressant",
];

const COMMON_INTERACTING_PAIRS: Array<[string, string, string]> = [
...
export function computeRisk(input: RiskInput): RiskResult {
  ...
  const meds = input.medications.map(m => m.trim().toLowerCase()).filter(Boolean);
...
function generateQuestions(input: RiskInput, factors: RiskFactor[]): string[] {
  const q: string[] = [];
  if (factors.some(f => f.label.includes("interaction"))) q.push("Are any of my medications known to interact, and is there a safer alternative?");
  if (factors.some(f => f.label.includes("Alcohol"))) q.push("How much alcohol, if any, is safe while I am taking these medications?");
  if (input.missedDosesPerWeek >= 2) q.push("If I miss a dose, what is the right way to catch up — should I double up or skip it?");
  if (input.inconsistentTiming) q.push("Does the exact time of day matter for these medications, and how strict do I need to be?");
  if (input.ageRange === "65+" || input.numberOfMedications >= 3) q.push("Can we review the full list together to see if any medication can be reduced or stopped?");
