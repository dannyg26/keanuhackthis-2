...
import { BODY_REGIONS, type BodySystem, systemsForMed, alertsForRegion, type AlertLevel } from "../data/bodyMap";
import {
  PillIcon, AlertIcon, SparklesIcon, BookIcon, XIcon, ArrowRightIcon, HeartPulseIcon,
...
interface SystemDef { id: AnatomySystem; label: string; color: string; }
...
export default function BodyMap() {
  ...
  const regionAlerts = useMemo(() => {
    if (!activeId) return [];
    return alertsForRegion(activeId, meds.map(m => m.name));
  }, [activeId, meds]);
  ...
  if (loading) {
    return (
      <>
        <PageHeader eyebrow="Body Map" title="Tap a glowing point on your body" />
    ...
    <>
      ...
      <div className="grid lg:grid-cols-6 gap-4">
          ...
          <div className="card bg-charcoal-800 text-white p-4 sticky lg:top-4">
            ...
            <ul className="space-y-1">
              <li>
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-xl text-left text-sm font-medium transition-colors ${
                    systemFilter === "all" ? "bg-white text-charcoal-900" : "text-white/70 hover:bg-white/10"
                  }`}
                >
              ...
              {SYSTEMS.map(s => (
                <li key={s.id}>
                  <button
                    onClick={() => setSystemFilter(prev => prev === s.id ? "all" : s.id)}
                    className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-xl text-left text-sm font-medium transition-colors ${
                      systemFilter === s.id ? "bg-white text-charcoal-900" : "text-white/70 hover:bg-white/10"
                    }`}
                  >
        ...
        <div className="lg:col-span-3 order-1 lg:order-2 card relative overflow-hidden p-0 bg-white border-ink-100">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,#f8fafc_0%,#ffffff_60%)]" aria-hidden />

          <div className="relative h-[600px] sm:h-[720px] flex items-center justify-center">
            <BodyMapAnatomy
              activeId={activeId}
              onSelectZone={(id) => setActiveId(prev => (prev === id ? null : id))}
            />
            ...
            {activeRegion && (
              ...
              >
                <span className="w-2 h-2 rounded-full" style={{ background: activeRegion.color }} />
                {activeRegion.name}
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span
                        ...
                        style={{
                          background: isActive ? "rgba(255,255,255,0.25)" : `${r.color}20`,
                        }}
                      >
                        {count}
                      </span>
        ...
        <aside className="lg:col-span-2 order-2 lg:order-3 space-y-4">
          {!activeRegion ? (
            <div className="card bg-cream-100 border-cream-200">
              ...
              <p className="text-sm text-ink-600 mt-2 leading-relaxed">
                Each point is a system DoseWise tracks. Click it to see related conditions, your medications that affect it, and labs to keep an eye on.
              </p>

              <div className="mt-5 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-blush-200 ring-1 ring-blush-300 p-3">
                  <p className="text-[10px] uppercase tracking-wider font-bold text-charcoal-800/65">Systems</p>
                  <p className="text-2xl font-extrabold text-charcoal-900 leading-none mt-1">{BODY_REGIONS.length}</p>
                </div>
                <div className="rounded-2xl bg-butter-200 ring-1 ring-butter-300 p-3">
                  <p className="text-[10px] uppercase tracking-wider font-bold text-charcoal-800/65">Your meds</p>
                  <p className="text-2xl font-extrabold text-charcoal-900 leading-none mt-1">{meds.length}</p>
                </div>
              </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {sys.length === 0 ? (
                            ...
                            sys.map(s => {
                              const region = BODY_REGIONS.find(r => r.id === s)!;
                              return (
                                <button
                                  key={s}
            ...
            <div
              ...
              style={{ borderColor: `${activeRegion.color}55`, background: `${activeRegion.color}0d` }}
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-[10px] uppercase tracking-[0.18em] font-bold" style={{ color: activeRegion.color }}>
                    Selected system
                  </p>
                  <h2 className="text-2xl font-extrabold text-charcoal-900 mt-1">{activeRegion.name}</h2>
                </div>
                <span
              ...
              <p className="text-sm text-ink-700 mt-2 leading-relaxed">{activeRegion.description}</p>

              {/* Organ illustration with label callout */}
              <div className="mt-4 relative rounded-2xl bg-charcoal-900 p-4 overflow-hidden">
                <div className="absolute inset-0 bg-mesh opacity-10" aria-hidden />
                ...
                <div className="absolute top-3 left-3 max-w-[55%]">
                  <div className="bg-white/95 backdrop-blur rounded-lg px-2.5 py-1.5 shadow-soft">
                    <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: activeRegion.color }}>
                      {activeRegion.name}
                    </p>
                  ...
                  <svg className="absolute left-6 top-full" width="80" height="40">
                    <line x1="0" y1="0" x2="60" y2="35" stroke="white" strokeWidth="1.2" opacity="0.6" />
              ...
              {regionAlerts.length > 0 && (
                <div className="mt-5 space-y-2">
                  <div className="flex items-center gap-2">
                    <AlertIcon className="w-4 h-4" style={{ color: activeRegion.color }} />
                  ...
                  <ul className="space-y-1.5">
                    {regionAlerts.map((a, i) => {
                      const colorMap: Record<AlertLevel, { bg: string; text: string; ring: string; chip: string }> = {
                        danger:  { bg: "bg-coral-50",  text: "text-coral-600",  ring: "ring-coral-200",  chip: "bg-coral-500 text-white" },
                        warning: { bg: "bg-sun-50",    text: "text-sun-500",    ring: "ring-sun-100",    chip: "bg-sun-400 text-charcoal-900" },
                        info:    { bg: "bg-sky2-50",   text: "text-sky2-600",   ring: "ring-sky2-100",   chip: "bg-sky2-500 text-white" },
                      };
                      const c = colorMap[a.level];
                      return (
                        <li key={i} className={`p-3 rounded-xl ring-1 ${c.bg} ${c.ring}`}>
                          <div className="flex items-start justify-between gap-2">
                            <p className={`text-sm font-bold ${c.text}`}>{a.title}</p>
                            <span className={`pill text-[9px] uppercase tracking-wider ${c.chip}`}>{a.level}</span>
                          </div>
                          <p className="text-xs text-ink-700 leading-snug mt-1">{a.detail}</p>
                          {a.triggeredBy && (
                            <p className="text-[10px] text-ink-400 mt-1">Triggered by: <span className="font-semibold">{a.triggeredBy}</span></p>
                          )}
                        </li>
                      );
                    })}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {activeRegion.symptoms.map(s => (
                    <span
                      ...
                      style={{ color: activeRegion.color, borderColor: `${activeRegion.color}40` }}
                    >
                      {s}
                    </span>
                  ))}
              ...
              <div className="mt-5">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <PillIcon className="w-4 h-4" style={{ color: activeRegion.color }} />
                    <p className="text-[11px] uppercase tracking-[0.18em] font-bold text-charcoal-800/65">
              ...
              <div className="mt-5">
                <div className="flex items-center gap-2 mb-2">
                  <BookIcon className="w-4 h-4" style={{ color: activeRegion.color }} />
                  <p className="text-[11px] uppercase tracking-[0.18em] font-bold text-charcoal-800/65">
                    Labs to ask about
                  </p>
                </div>
          ...
          <div className="rounded-2xl border border-ink-100 bg-white p-4 text-xs text-ink-600 leading-relaxed">
            This map is a learning aid, not a diagnosis. Discuss any new symptoms or medication concerns with a licensed provider.
          </div>
        </aside>
      </div>
      ...
      {activeRegion && (
        <div className="mt-6 card bg-blush-50 border-blush-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="pill bg-ink-100 text-ink-600">Hover to highlight regions</span>
          </div>
              ...
              return (
                <li
                  ...
                  className={`p-3 rounded-xl border-2 transition ${
                    active ? "border-current bg-white" : "border-ink-100 bg-white"
                  }`}
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="font-semibold text-ink-900 truncate">{m.name}</p>
                    {active && (
                      <span className="pill ring-1 text-[10px]" style={{
                        background: `${activeRegion.color}1a`,
                        color: activeRegion.color,
                        borderColor: `${activeRegion.color}40`,
                      }}>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    ...
                    {sys.length === 0 && <span className="text-[10px] text-ink-400">no system mapped</span>}
                  </div>
                </li>
              );
            })}
