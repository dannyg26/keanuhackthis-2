...
import {
  ShieldIcon, PlusIcon, XIcon, AlertIcon, SparklesIcon, MoonIcon, CoffeeIcon, ClockIcon,
...
export default function RiskEngine() {
  ...
  useEffect(() => {
    ...
    api.medications.list()
      .then(({ medications }) => {
        ...
        setInput(prev => ({ ...prev, medications: names, numberOfMedications: names.length }));
      })
      .catch(() => { /* keep defaults */ });
  ...
  return (
    <>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="input"
              />
              <button type="button" onClick={addMed} className="btn-secondary px-3">
                <PlusIcon className="w-4 h-4" /> Add
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {input.medications.map((m, i) => (
                <span key={i} className="pill bg-brand-50 text-brand-700 ring-1 ring-brand-100 pr-1.5">
                  {m}
                  <button type="button" onClick={() => removeMed(i)} className="ml-1 hover:text-coral-600">
                    <XIcon className="w-3.5 h-3.5" />
                  </button>
                </span>
              ))}
              {input.medications.length === 0 && (
                <p className="text-sm text-ink-400">Add at least one medication to get started.</p>
              )}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="w-16 text-center font-bold text-ink-900">{input.sleepHours}h</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={e => update("missedDosesPerWeek", parseInt(e.target.value))}
                  className="flex-1 accent-brand-500"
                />
                <span className="w-16 text-center font-bold text-ink-900">{input.missedDosesPerWeek}</span>
              </div>
            </div>
            ...
            <div>
              <label className="label flex items-center gap-2">
                ...
                I take meds at inconsistent times
              </label>
        ...
        <aside className="lg:col-span-2 space-y-4">
          <div className="card bg-gradient-to-br from-butter-50 to-cream-50 border-butter-200">
            ...
            <ul className="mt-3 text-sm space-y-1.5 text-ink-700">
              <li>· Drug interaction found <span className="text-ink-400">+30</span></li>
              <li>· Alcohol with sedating med <span className="text-ink-400">+25</span></li>
              <li>· Frequent missed doses <span className="text-ink-400">+20</span></li>
              <li>· Sleep under 5 hours <span className="text-ink-400">+15</span></li>
              <li>· Inconsistent timing <span className="text-ink-400">+15</span></li>
              <li>· Age 65 or older <span className="text-ink-400">+15</span></li>
              <li>· 3+ medications <span className="text-ink-400">+10</span></li>
            </ul>
            <p className="text-xs text-ink-400 mt-3 leading-relaxed">
              Capped at 100. Every point on your score traces back to one of these rules — no black boxes.
            </p>
          </div>
          ...
          <div className="card bg-coral-50 border-coral-100">
            ...
            <p className="mt-2 text-sm text-ink-600 leading-relaxed">
              DoseWise is a risk-awareness tool. It does not diagnose, prescribe, or replace medical advice
      ...
      {/* Result */}
      {result && (
        <div id="risk-result" className="mt-8 grid lg:grid-cols-5 gap-6 animate-slideUp">
          <div className={`lg:col-span-2 card flex flex-col items-center justify-center text-center ${tone?.bg ?? ""}`}>
            <RiskGauge score={result.score} size={220} />
            <p className="mt-4 text-sm text-ink-600 max-w-xs">
              Your score is <span className="font-bold">{result.score}</span> out of 100, which we
              classify as <span className={`font-bold ${tone?.text}`}>{result.level} risk</span>.
            </p>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="card bg-cream-50 border-cream-200">
              ...
              <ul className="mt-3 space-y-2">
                {result.factors.length === 0 ? (
                  <li className="text-mint-500 font-medium">No major risk factors detected — great work.</li>
                ) : result.factors.map(f => (
                  <li key={f.label} className="p-3 rounded-xl bg-ink-50 flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-ink-900">{f.label}</p>
                      <p className="text-sm text-ink-600 leading-snug">{f.detail}</p>
                    </div>
                    <span className="pill bg-white text-coral-600 ring-1 ring-coral-100 shrink-0">+{f.points}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="card bg-butter-50 border-butter-200">
              <p className="section-title">Questions to ask your doctor or pharmacist</p>
              <ul className="mt-3 space-y-2">
                {result.questions.map((q, i) => (
                  <li key={i} className="flex items-start gap-3 p-3 rounded-xl border border-ink-100 bg-white">
            ...
            <div className="rounded-2xl border border-coral-100 bg-coral-50 p-4 text-sm text-coral-600">
              <strong>Disclaimer:</strong> This score is informational only and does not replace medical advice.
              Talk to a licensed healthcare provider before making any medication change.
            </div>
          </div>
        </div>
      )}
