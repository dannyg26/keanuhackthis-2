...
export default function MedGuide() {
  ...
  const [addForm, setAddForm] = useState({
    name: "", dosage: "", frequency: "", category: "", refillsLeft: "",
  });
  const [addBusy, setAddBusy] = useState(false);
  const [addError, setAddError] = useState<string | null>(null);
  const [addEnriching, setAddEnriching] = useState(false);
  ...
  const saveManualMed = async () => {
    ...
    try {
      setAddEnriching(true);
      const { enrichment } = await api.medications.enrich({ name: addForm.name.trim() });
      purpose = enrichment.purpose;
      sideEffects = enrichment.sideEffects;
      callDoctor = enrichment.callDoctor;
    } catch {
      purpose = "Ask your pharmacist for details about this medication.";
      sideEffects = ["Nausea", "Headache", "Drowsiness", "Upset stomach"];
      callDoctor = ["Severe allergic reaction", "Difficulty breathing", "Symptoms worsen", "Unusual side effects"];
    } finally {
      setAddEnriching(false);
    }

    let refills: number | null = null;
    if (addForm.refillsLeft.trim() !== "") {
      const n = parseInt(addForm.refillsLeft, 10);
      if (!Number.isNaN(n) && n >= 0 && n <= 99) refills = n;
    }

    try {
      const { medication } = await api.medications.create({
        name: addForm.name.trim(),
        dosage: addForm.dosage.trim() || "See label",
        frequency: addForm.frequency.trim() || "As directed",
        purpose,
      ...
      setAddForm({ name: "", dosage: "", frequency: "", category: "", refillsLeft: "" });
      setAddOpen(false);
    } catch (e) {
      ...
      <>
        ...
        <div className="card text-center py-16 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-ink-500 mt-1">Scan a bottle or enter one manually — AI will fill in the details.</p>
          </div>
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <button onClick={() => setScannerOpen(true)} className="btn-primary px-5 py-3">
              <ScanIcon className="w-4 h-4" /> Scan a bottle
            </button>
            <button onClick={() => setAddOpen(true)} className="btn-secondary px-5 py-3">
        ...
        {addOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-soft w-full max-w-md p-6 space-y-4">
              <div className="flex items-center justify-between">
                <p className="font-bold text-charcoal-900">Add medication manually</p>
                <button onClick={() => { setAddOpen(false); setAddError(null); }} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-ink-100">
                  <XIcon className="w-4 h-4" />
                </button>
              </div>
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-semibold text-ink-600 block mb-1">Medication name <span className="text-coral-500">*</span></label>
                  <input
                    value={addForm.name}
                    onChange={(e) => setAddForm((f) => ({ ...f, name: e.target.value }))}
                    className="input w-full text-sm"
                    placeholder="e.g. Lisinopril, Tylenol, Metformin"
                    autoFocus
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={addForm.refillsLeft}
                      onChange={(e) => setAddForm((f) => ({ ...f, refillsLeft: e.target.value }))}
                      className="input w-full text-sm"
                      placeholder="—"
                    />
                  </div>
                ...
                {addEnriching && (
                  <p className="text-xs text-brand-700 flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-full border-2 border-brand-200 border-t-brand-700 animate-spin inline-block" />
                    AI is filling in side effects and details…
                  </p>
                )}
                {addError && <p className="text-xs text-coral-600">{addError}</p>}
              </div>
              <div className="flex gap-2 justify-end pt-1">
                <button onClick={() => { setAddOpen(false); setAddError(null); }} disabled={addBusy} className="btn-secondary">Cancel</button>
                <button onClick={saveManualMed} disabled={addBusy || !addForm.name.trim()} className="btn-primary disabled:opacity-50">
                  {addBusy ? "Adding…" : "Add medication"}
                </button>
              </div>
            </div>
          </div>
  ...
  return (
    <>
      ...
      {/* Manual add modal (when meds already exist) */}
      {addOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-soft w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
