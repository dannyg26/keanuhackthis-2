...
export interface AICharge {
  name: string; amount: number; category: string; explanation: string;
}
export interface AIIssue {
  type: string; description: string; severity: "warning" | "info"; relatedCharges?: string[];
}
export interface BenchmarkHit {
  chargeName: string; category: string; amount: number;
  benchmarkName: string; low: number; high: number;
  multiple: number; potentialSavings: number;
  severity: "ok" | "watch" | "high"; confidence: number;
}
export interface BillAudit {
  score: number; potentialSavings: number;
...
export interface AIBillAnalysis {
  summary: string; charges: AICharge[]; total: number; issues: AIIssue[]; questions: string[]; score: number; audit: BillAudit;
}

export interface OTCRecommendation {
  name: string; generic: string; what_for: string; dosage: string; typical_price: string; caution?: string;
}
export interface TriageResult {
  ...
  questions_to_ask: string[]; what_to_bring: string[]; red_flags: string[];
  estimated_cost: string; otc_recommendations: OTCRecommendation[]; self_care_tips: string[];
}

export interface CoveragePath {
  name: string; match_score: number; best_for: string; why_it_fits: string;
  watch_out: string; next_step: string; website?: string;
}
export interface CoverageAlert {
  title: string; severity: "high" | "medium" | "low"; detail: string; question_to_ask: string;
}
export interface CoverageResult {
  headline: string; summary: string; coverage_paths: CoveragePath[]; plan_fit_score: number;
  plan_fit_label: string;
  estimated_yearly_scenarios: Array<{ scenario: string; likely_cost: string; note: string }>;
  risk_alerts: CoverageAlert[]; questions_to_ask: string[]; community_angle: string;
}

export interface PlaceInsightInput {
  id: string; name: string; type: string; brand?: string; distance: string; open?: "open" | "closed" | "unknown";
}
export interface PlaceInsight {
  id: string; insurance_match?: string; insurance_note?: string;
  cost_estimate: string; wait_estimate: string; fit_score?: number; fit_reason?: string;
}
...
export interface DenialExplainResult {
  denial_reason: string; plain_english: string; why_this_happens: string; your_rights: string;
  appeal_steps: string[]; documents_needed: string[]; call_script: string[];
  success_likelihood: "high" | "medium" | "low"; success_label: string;
}
export interface DenialAppealResult { subject_line: string; letter_text: string; }
...
export const api = {
  ...
  bills: {
    ...
    aiAnalyze: (body: { rawText?: string; imageBase64?: string; imageMediaType?: string }) =>
      request<{ analysis: AIBillAnalysis }>("/api/bills/ai-analyze", { method: "POST", body: JSON.stringify(body) }),
  ...
  triage: {
    analyze: (body: { symptoms: string; allergies?: string; medications?: string; history?: string }) =>
      request<TriageResult>("/api/triage", { method: "POST", body: JSON.stringify(body) }),
  },

  coverage: {
    analyze: (body: Record<string, string>) =>
      request<CoverageResult>("/api/coverage", { method: "POST", body: JSON.stringify(body) }),
  },

  denial: {
    explain: (body: { text?: string; imageData?: string; imageType?: string }) =>
      request<DenialExplainResult>("/api/denial-explain", { method: "POST", body: JSON.stringify(body) }),
    appeal: (body: { denialText?: string; analysis: DenialExplainResult }) =>
      request<DenialAppealResult>("/api/denial-appeal", { method: "POST", body: JSON.stringify(body) }),
  },
  ...
  placeInsights: {
    score: (body: {
      ...
      insurance?: string;
      triage?: { care_type: string; urgency: string; reasoning: string; symptoms?: string } | null;
    }) => request<{ insights: PlaceInsight[] }>("/api/place-insights", { method: "POST", body: JSON.stringify(body) }),
  },
};
