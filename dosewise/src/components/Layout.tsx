...
import { HomeIcon, ShieldIcon, PillIcon, ReceiptIcon, BookIcon, TagIcon, BodyIcon, XIcon, MapPinIcon, InsuranceIcon, CubeIcon } from "./Icon";
...
const toolItems = [
  ...
  { to: "/find-care",  label: "Find Care",    icon: MapPinIcon,    short: "Care"      },
  { to: "/insurance",  label: "Insurance",    icon: InsuranceIcon, short: "Insurance" },
];

// Mobile bottom nav: simplified to 6 items — general minus Body, plus Tools hub
const mobileNavItems = [
  { to: "/dashboard", icon: HomeIcon,    short: "Home"  },
  { to: "/medguide",  icon: BookIcon,    short: "Guide" },
  { to: "/adherence", icon: PillIcon,    short: "Doses" },
  { to: "/bills",     icon: ReceiptIcon, short: "Bills" },
  { to: "/body",      icon: BodyIcon,    short: "Body"  },
  { to: "/tools",     icon: CubeIcon,    short: "Tools" },
];
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-mesh bg-cream-50">
      ...
      {/* Mobile bottom nav — dark pill, 6 items */}
      <nav className="lg:hidden fixed bottom-3 inset-x-3 z-30 bg-charcoal-800 text-white rounded-3xl shadow-soft grid grid-cols-6 overflow-hidden">
        {mobileNavItems.map(({ to, icon: Icon, short }) => (
          <NavLink
            key={to}
            to={to}
            className={({ isActive }) =>
              `flex flex-col items-center justify-center gap-1 py-3 text-[10px] font-semibold transition-colors ${
